...
void CWorm::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	if (Allow[JASON] || Allow[SOPHIA]) {
		...
		if (StateObject != ENEMY_DEAD) {
			if (isWalking)
			{
				if (x <= cx && this->StateObject != WORM_STATE_WALKING_RIGHT)
				{
					ChangeAnimation(WORM_STATE_WALKING_RIGHT);
				}
				else if (x > cx && this->StateObject != WORM_STATE_WALKING_LEFT)
				{
			...
			if (kc <= 150 && isDrop)
			{
				if (x > cx && this->StateObject != WORM_STATE_DROP_LEFT)
				{
					ChangeAnimation(WORM_STATE_DROP_LEFT);
				}
				else if (x <= cx && this->StateObject != WORM_STATE_DROP_RIGHT)
				{
